...
export default function ConnectIntegrations() {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-3xl md:text-4xl lg:text-[50px] font-degular font-bold text-black">
          Connect What You Already Use
        </h2>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-6 px-4 md:px-8 lg:px-12">
        ...
        {/* Person Image — hidden on mobile, shown lg+ */}
        <div
          className="hidden lg:block w-[45%] flex-shrink-0"
          style={{
            opacity: visible ? 1 : 0,
            transform: visible ? "translateX(0)" : "translateX(-30px)",
            transition: "opacity 0.7s ease 0.2s, transform 0.7s ease 0.2s",
        ...
        <div className="flex-1 flex flex-col gap-4 mb-10">
          ...
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-5">
            {INTEGRATIONS.slice(0, 3).map((integration) => (
             ...
             {/* Bottom card joins the grid on screens < lg */}
            <div className="block lg:hidden">
              <IntegrationCard integration={INTEGRATIONS[3]} visible={visible} />
            </div>
          </div>

          {/* Bottom Row — stack on mobile, inline on lg */}
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 lg:gap-6 mt-4 lg:mt-6">
            {/* Bottom card only visible on lg+ (stay below grid) */}
            ...
            {/* CTA Buttons */}
            <div
              className="flex flex-wrap items-center justify-center lg:justify-stretch gap-3  w-full"
              style={{
                opacity: visible ? 1 : 0,
                transition: "opacity 0.6s ease 0.5s",
              }}
...
function IntegrationCard({ integration, visible }) {
  ...
  return (
    <div
      ...
      className="bg-transparent py-4 px-4 lg:px-6 rounded-2xl border flex flex-col gap-3 cursor-pointer"
    >
      <div className="flex items-center justify-center w-full">
        {integration.icon}
      </div>
      <div className="text-center flex flex-col items-center justify-center">
        <p className="text-sm lg:text-base font-bold text-black">{integration.label}</p>
        {integration.sublabel && (
          <p className="text-xs lg:text-sm text-black text-center text-wrap mt-0.5 px-2 lg:px-4">{integration.sublabel}</p>
        )}
      </div>
    </div>
  );
